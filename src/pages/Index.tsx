...
async function scanNativeNetworks(): Promise<WifiNetwork[]> {
  try {
    ...
    const mod = await import('@capgo/capacitor-wifi') as any;
    const WifiManager = mod.WifiManager || mod.default?.WifiManager || mod.default;
...
export default function Index() {
  ...
  const scan=useCallback(async()=>{
    setScanning(true);
    setNetworks(isNative?await scanNativeNetworks():generateNetworks());
    setScanning(false);
  },[isNative]);

  useEffect(()=>{scan();const t=setInterval(scan,scanInterval*1000);return()=>clearInterval(t);},[scan]);
  ...
  const exportData=()=>{
    const csv=['SSID,BSSID,Signal,Channel,Band,Frequency,Security,Vendor',...filtered.map(n=>`"${n.ssid}","${n.bssid}",${n.dbm},${n.channel},"${n.band}",${n.frequency},"${n.security}","${n.vendor}"`)].join('\n');
    const a=document.createElement('a');
    a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));
    a.download=`wifi-${Date.now()}.csv`;a.click();
  };

  const fb=(active:boolean):React.CSSProperties=>({padding:'4px 10px',borderRadius:'999px',border:'1px solid',borderColor:active?'#3b82f6':'#334155',background:active?'#1d4ed8':'transparent',color:active?'#fff':'#94a3b8',cursor:'pointer',fontSize:'11px',whiteSpace:'nowrap'});
    ...
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#f1f5f9',fontFamily:'monospace',display:'flex',flexDirection:'column'}}>
      <div style={{borderBottom:'1px solid #1e293b',padding:'8px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:10,background:'#0f172a'}}>
        ...
        <div style={{display:'flex',gap:'8px'}}>
          <button style={{display:'flex',alignItems:'center',gap:'4px',padding:'4px 10px',borderRadius:'6px',border:'1px solid #334155',background:'transparent',color:'#94a3b8',cursor:'pointer',fontSize:'11px'}} onClick={exportData}><Download size={12}/>CSV</button>
      ...
      <div style={{display:'flex',gap:'8px',padding:'8px 16px',borderBottom:'1px solid #1e293b',overflowX:'auto',flexShrink:0}}>
        <span style={{fontSize:'11px',color:'#64748b',alignSelf:'center'}}>Band:</span>
        {(['all','2.4GHz','5GHz'] as const).map(b=><button key={b} style={fb(band===b)} onClick={()=>setBand(b)}>{b}</button>)}
        <span style={{fontSize:'11px',color:'#64748b',alignSelf:'center',marginLeft:'8px'}}>Sec:</span>
        {(['all','Open','Secured'] as const).map(s=><button key={s} style={fb(security===s)} onClick={()=>setSecurity(s)}>{s}</button>)}
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'8px'}}>
        {filtered.length===0?<div style={{textAlign:'center',padding:'48px',color:'#475569'}}><p>No networks detected</p></div>:filtered.map(n=>(
          <div key={n.id} style={{background:'#1e293b',borderRadius:'8px',padding:'12px',marginBottom:'8px',border:'1px solid #334155'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'6px'}}>
              <div><div style={{fontWeight:600,fontSize:'14px'}}>{n.ssid}</div><div style={{fontSize:'11px',color:'#64748b'}}>{n.bssid}</div></div>
              <div style={{textAlign:'right'}}><SignalBar dbm={n.dbm}/><div style={{fontSize:'12px',color:'#94a3b8',marginTop:'4px'}}>{n.dbm}dBm</div></div>
            </div>
      ...
      <div style={{display:'flex',gap:'16px',padding:'8px 16px',borderTop:'1px solid #1e293b',fontSize:'11px',color:'#64748b'}}>
        <span>{filtered.length} networks</span><span>Every {scanInterval}s</span>
        <span style={{marginLeft:'auto'}}>{isNative?'📱 Native':'🌐 Sim'}</span>
      </div>
